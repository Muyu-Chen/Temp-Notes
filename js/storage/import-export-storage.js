...
const normalizeExportItems = (items) =>
  (Array.isArray(items) ? items : [])
    .filter((x) => x && typeof x === "object")
    .map(normalizeItem)
    .filter((x) => x.content.length > 0 || x.attachments.length > 0);

const normalizeExportRecycle = (items) =>
  (Array.isArray(items) ? items : [])
    .filter((x) => x && typeof x === "object")
...
export const exportData = (
  ...
  items: normalizeExportItems(items),
  recycle: normalizeExportRecycle(recycle),
});
...
export const normalizeImportedData = (data) => {
  ...
  const importedRecycle = Array.isArray(data.recycle) ? data.recycle : [];
