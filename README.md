...
### TODO
- [ ] 改为 IndexedDB 存储：提供更大的存储空间，并确保所有客户端（不同浏览器、不同设备）都能正常使用（不是同步，只是为了确保所有用户都能正常使用功能）。  
- [ ] 增加对称加密：为数据提供密码保护功能，确保数据的隐私性和安全性。用户可以设置密码来加密/解密数据。
- [ ] 草稿区与存档条目合并：如果新保存的草稿与之前才保存的条目开头一致，则自动更新为同一条记录，而不是创建新条目。如果打开的草稿修改后保存，需要覆盖保存。需要判断文本开头的一致性，合并逻辑需清晰。
