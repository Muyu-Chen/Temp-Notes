...
### TODO - 待开发
- [ ] 增加打开页面时的加载动画。
- [ ] 新增默认密码修改功能，目前默认密码为“password”，用户可以在设置中修改默认密码，修改后新加密的条目将使用新的默认密码，旧密码请用户自行管理，修改默认密码不会影响之前已加密的条目的密码（即之前密码留空的加密条目仍然使用旧默认密码“password”）。
- [ ] 上一条功能基础上，新增“一键迁移密码”功能，用户可以选择将之前默认密码的加密条目批量迁移到新的默认密码，迁移后这些条目的密码将更新为新的默认密码，用户无需逐条修改密码（请一定提前备份数据，迁移过程中如果出现问题可以恢复到之前的状态）。
...
- [ ] 基于可选 LLM 配置新增摘要、润色、标签等 AI 草稿能力。
