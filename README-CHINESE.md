...
一个**默认完全离线运行**的临时笔记应用，支持快速草稿、条目存档、Markdown 预览、数据导入导出。所有笔记数据默认存储在本地浏览器；只有用户主动启用大模型功能并点击测试连接或使用未来 AI 功能时，才会请求用户自己配置的服务。
...
- **tips**：大模型功能默认关闭。连接测试只会在用户启用大模型并点击测试按钮后，请求 `GET {base_url}/models`；`base_url`、`api_key`、`model` 均由用户输入并保存在本地。
...
- 可选的大模型连接测试只会在用户明确点击后使用 `fetch()` 请求用户配置的地址。
...
| `services/llm-service.js` | 可选大模型连接测试 | `LLMService` |
...
- 导入导出工具：数据规范化、空条目过滤、去重、排序、导出结构
- 单条 TXT/Markdown 导出：文件名清理、时间戳、MIME 类型、加密条目菜单限制
- 设置项：Markdown 模式记忆、回收站保留策略、LLM 设置保存
- LLM 连接服务：关闭或配置不完整时不请求、OpenAI-compatible `/models` 请求、成功/失败状态
...
- [ ] 增加条目置顶与收藏，让常用临时笔记固定在列表前方，同时不影响普通条目的更新时间排序。
- [ ] 增加轻量标签与标签筛选；标签数据保持本地存储，并纳入 JSON 导入/导出。
- [ ] 增加草稿版本历史，保留最近几次编辑，并支持误覆盖后的简单恢复。
- [ ] 优化大体量本地数据库的首次加载性能，并增加克制的加载状态。

#### 录音与附件

- [ ] 增加本地录音功能：在草稿区使用浏览器录音 API，支持开始、暂停、继续、停止、录音时长显示，并默认仅保存在本地。
- [ ] 支持将录音作为附件保存到存档条目，纳入 JSON 备份，并在存储统计中显示附件体积。
- [ ] 在条目中增加录音播放控件，支持进度拖动、倍速播放、重命名和移除附件。
- [ ] 支持图片附件：提供本地预览、体积提示，并兼容 JSON 备份。
...
- [ ] 基于可选 LLM 配置新增摘要、润色、续写、提取待办、自动标签等草稿能力。
